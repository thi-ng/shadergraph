...
The brief example below defines a simple shader spec with standard
Lambert lighting. The =assemble= function takes a map of GLSL
dependencies and source code and returns a transformed source w/ all
transitive deps injected in the correct order. Furthermore, the
=minify= macro is used to shrink the original sources (all snippets
provided by this library are minified already).
...
#+BEGIN_SRC clojure
  ...
  (def shader-spec
    {:vs
     (glsl/assemble
      {:deps
       [js/thi.ng.glsl.vertex.mvp
        js/thi.ng.glsl.vertex.normal]
       :src
       (minified
        "void main() {
     ...
     (glsl/assemble
      {:deps
       [js/thi.ng.glsl.lighting.lambert]
       :src
       (minified
        "void main() {
...
For example, using [[https://github.com/thi-ng/geom/blob/master/geom-webgl/src/index.org][geom-webgl]] this shader spec can then be compiled
into a full shader program just like this (the function injects all
attribute, uniform & varying definitions automatically):

#+BEGIN_SRC clojure
  (require '[thi.ng.geom.webgl.shaders :as shader])

  (def program
    (shader/make-shader-from-spec my-gl-context example/shader-spec))
#+END_SRC
...
[kibu/dependency "0.1.2-SNAPSHOT"]
#+END_SRC
