...
#+BEGIN_SRC clojure
  (ns example
    (:require
     [thi.ng.glsl.core :as glsl :include-macros true :refer-macros [minified]]
  ...
  (def shader-spec
    {:vs
     (glsl/assemble
      {:deps
       [thi.ng.glsl.vertex.mvp/spec
        thi.ng.glsl.vertex.normal/spec]
       :src
       (minified
        "void main() {
     ...
     (glsl/assemble
      {:deps
       [thi.ng.glsl.lighting.lambert/spec]
       :src
       (minified
        "void main() {
...
into a full shader just like this (the function injects all
attribute, varying & uniform definitions and setters automatically):

#+BEGIN_SRC clojure
  (require '[thi.ng.geom.webgl.shaders :as s])

  (def shader
    (s/make-shader-from-spec my-gl-context example/shader-spec))
#+END_SRC
